...
iTalentConnect`
  ...
  const handleDownloadCV = async (candidate) => {
    ...
    const subParts = [candidate.gender, candidate.currentLocation, candidate.phoneNumber, candidate.email].filter(Boolean);
    ...
    doc.text('CAREER OBJECTIVE', margin, y); y += 14;
    doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(80, 80, 80);
    const objective = candidate.remark || candidate.reasonforLeaving || 'I aim to be associated with an organization that gives me the scope to apply my skills and actively work towards the growth of the organization and myself.';
    const objLines = doc.splitTextToSize(objective, W - margin * 2);
    doc.text(objLines, margin, y); y += objLines.length * 13 + 16;

    doc.setFont('helvetica', 'bold'); doc.setFontSize(12); doc.setTextColor(40, 40, 40);
    doc.text('EDUCATION', margin, y); y += 14;
    doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(80, 80, 80);
    if (candidate.qualification) { doc.text(`Education level: ${candidate.qualification}`, margin, y); y += 13; }
    y += 10;

    if (candidate.currentPosition || candidate.currentCompany || candidate.experience) {
      doc.setFont('helvetica', 'bold'); doc.setFontSize(12); doc.setTextColor(40, 40, 40);
      doc.text('WORK EXPERIENCE', margin, y); y += 14;
      doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(80, 80, 80);
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#f0f2f8" }}>
      ...
      <Box sx={{ flexGrow: 1, display: "flex", flexDirection: "column", ml: "250px", minHeight: "100vh" }}>
        ...
        <Box sx={{ display: "flex", flexGrow: 1, overflow: "hidden", height: "calc(100vh - 64px)" }}>
          ...
          <Box sx={{
            ...
            <Box sx={{ px: 3, pb: 3, overflowY: "auto", flexGrow: 1,
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)}
        ...
        <Box sx={{ background: 'linear-gradient(135deg, #2563eb 0%, #3b82f6 100%)', px: 3, py: 2,
          display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
        ...
        <DialogContent sx={{ p: 3, bgcolor: '#f8fafc' }}>
          <Grid container spacing={2}>
            {[
              ...
              { key: 'positionName',      label: 'Position Applied For' },
        ...
        <DialogActions sx={{ px: 3, py: 2, bgcolor: '#f8fafc', borderTop: '1px solid #e8eaf6', gap: 1 }}>
          ...
          <Button variant="contained" onClick={handleEditSave} disabled={editSaving}
            ...
            {editSaving ? <CircularProgress size={20} sx={{ color: '#fff' }} /> : 'Save Changes'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}
        ...
        <Box sx={{ background: 'linear-gradient(135deg, #e11d48 0%, #be123c 100%)', px: 3, py: 2.5,
                ...
                <Box>
                  {selectedCandidate.resumeUpload && selectedCandidate.resumeUpload.startsWith('http') ? (
                    ...
                    <Box>
                      ...
                      <Box sx={{ bgcolor: '#fff', border: '1px solid #e0e0e0', borderRadius: '8px', overflow: 'hidden' }}>
                        ...
                        <Box sx={{ px: 3, py: 2 }}>
                          ...
                          <Typography fontSize={13} color="#475569" lineHeight={1.7} mb={2.5}>
                            {selectedCandidate.remark || selectedCandidate.reasonforLeaving || 'I aim to be associated with an organization that gives me the scope to apply my skills and actively work towards the growth of the organization and myself.'}
                          </Typography>
                          ...
                          <Typography fontSize={13} color="#64748b" mb={2.5}>Education level: {selectedCandidate.qualification || 'Graduate'}</Typography>
                        ...
                        <Box sx={{ bgcolor: '#f1f5f9', px: 3, py: 1, borderTop: '1px solid #e0e0e0' }}>
                          <Typography fontSize={11} color="#94a3b8" fontStyle="italic">This CV is generated by iTalentConnect from candidate's profile</Typography>
                        </Box>
                      </Box>
                    </Box>
                  )}
